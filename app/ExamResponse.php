...
class ExamResponse extends BaseModel {
    ...
    public function getSubjectSummaryAttribute() {
        $query = "
            select
                ...
                ifnull(answered.num_answered,0) as num_answered,
                ifnull(answered.num_correct,0) as num_correct,
                asked.num_questions - ifnull(answered.num_correct,0) as num_errors,
    ...
    public function getSummaryAttribute() {
        ...
        $query = "
            select
                asqued.exam_response_id as exam_response_id,
                asqued.num_questions as num_questions,
                ifnull(answered.num_answered,0) as num_answered,
                ifnull(answered.num_correct,0) as num_correct,
                asqued.num_questions - ifnull(answered.num_correct,0) as num_errors,
                asqued.max_errors as max_errors,
                case
                    when max_errors is null then 1
                    when max_errors >= asqued.num_questions - ifnull(answered.num_correct,0) then 1
                    else 0
                end as exam_passed
            ...
            on asqued.exam_response_id = answered.exam_response_id
        ";
